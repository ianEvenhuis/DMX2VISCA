...
package org.ianShowtechniek.io.artnet.api.packets;

import org.ianShowtechniek.io.artnet.api.ArtNet;
...
public class ArtNetPacketParser {
    ...
    public static ArtNetPacket createPacketForOpCode(int opCode, byte[] data) {
        ArtNet.logger.trace("creating packet instance for opcode: 0x"
        ...
        for (PacketType type : PacketType.values()) {
            if (opCode == type.getOpCode()) {
                ...
                if (packet != null) {
                    ...
                    ArtNet.logger.trace("packet type valid, but not yet supported: "
                            + type);
                }
            }
        }
    ...
    private static ArtNetPacket parse(byte[] raw, int offset, int length) {
        ...
        if (data.length > 10) {
            if (data.compareBytes(ArtNetPacket.HEADER, 0, 8) ||
                ...
                ArtNet.logger.warning("invalid header");
            }
        } else {
            ArtNet.logger.warning("invalid packet length: " + data.length);
        }
        return packet;
    }
