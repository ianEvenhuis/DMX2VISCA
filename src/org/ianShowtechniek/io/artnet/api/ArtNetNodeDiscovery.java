...
import org.ianShowtechniek.io.artnet.api.events.ArtNetDiscoveryListener;
import org.ianShowtechniek.io.artnet.api.packets.ArtPollPacket;
...
public class ArtNetNodeDiscovery implements Runnable {
	...
	public void discoverNode(ArtPollReplyPacket reply) {
		...
		if (node == null) {
			ArtNet.logger.info("discovered new node: " + nodeIP);
	...
	public void run() {
		try {
			...
			ArtNet.logger.error("node discovery interrupted " + e.getMessage());
		}
	}
